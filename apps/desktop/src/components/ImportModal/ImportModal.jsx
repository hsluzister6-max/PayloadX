...
export default function ImportModal() {
  ...
  return (
    <ModalShell
      ...
      subtitle="Postman collection import"
      maxWidth="max-w-lg"
      icon={importIcon}
    >
          {step === 'upload' && (
              ...
              <div
                ...
                className={`relative rounded-xl p-10 flex flex-col items-center justify-center gap-4 cursor-pointer transition-all duration-500 overflow-hidden group ${
                  isDragActive
                    ...
                    : 'border-2 border-dashed border-[var(--border-2)] hover:border-[var(--border-1)] hover:bg-[var(--surface-2)]'
                }`}
              >
                ...
                <div className={`relative z-10 w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-500 shadow-xl ${
                  isDragActive 
                    ...
                    : 'bg-[var(--surface-2)] border border-[var(--border-1)] group-hover:scale-105'
                }`}>
          ...
          {step === 'preview' && parsed && (
            <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-2 duration-500">
              {/* Summary */}
              <div className="bg-[var(--surface-2)] rounded-2xl p-5 border border-[var(--border-1)]">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-tx-primary">{parsed.collectionMeta.name}</h3>
              ...
              {/* Destination */}
              <div className="text-[11px] text-tx-secondary bg-[var(--surface-2)] border border-[var(--border-1)] rounded-xl px-4 py-3.5 flex items-center gap-2">
                <svg className="w-4 h-4 text-tx-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                <span>Importing to: <span className="text-tx-primary font-semibold">{currentTeam?.name}</span> → <span className="text-tx-primary font-semibold">{currentProject?.name || '(No project selected)'}</span></span>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setStep('upload')} className="btn-ghost flex-1 text-sm">
                  Cancel
                </button>
                <button
          ...
          {step === 'done' && (
            <div className="flex flex-col items-center gap-5 py-8 animate-in zoom-in-95 duration-500">
              ...
              <div className="text-center">
                <p className="text-xl font-bold text-tx-primary">Import Successful!</p>
                <p className="text-surface-400 text-sm mt-2">{parsed.requests.length} requests successfully supercharged.</p>
              </div>
              <button onClick={() => setShowImportModal(false)} className="btn-primary mt-2 px-10">
                Awesome
              </button>
            </div>
          )}
...
function Stat({ icon, label, value }) {
  return (
    <div className="flex-1 flex items-center gap-3 bg-[var(--surface-2)] border border-[var(--border-1)] rounded-xl p-3 hover:bg-[var(--surface-3)] transition-all duration-300">
      <div className="w-10 h-10 rounded-lg bg-[var(--surface-3)] flex items-center justify-center text-lg border border-[var(--border-1)]">
        {icon}
      </div>
      <div>
        <p className="text-lg font-bold text-tx-primary leading-none mb-1">{value}</p>
